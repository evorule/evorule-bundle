...
//!
//! SSOT：版本模型唯一来源在本 crate（evorule-bundle），evorule-rule 侧 re-export。
...
impl VersionSelectionMode {
    /// 序列化一致的小写 snake_case 字符串（SSOT，供执行侧溯源/落盘记录）
    pub fn as_str(&self) -> &'static str {
        match self {
            VersionSelectionMode::AutoByEffectiveDate => "auto_by_effective_date",
            VersionSelectionMode::Pinned => "pinned",
        }
    }
}
...
pub struct Versioning {
    ...
    ///
    /// 契约（types.ts GovernanceDataset.versioning）：必填数组 —— 空也必须输出 `[]`，
    /// 不得省略，否则前端 `{#each selected.versioning.chain}` 在 undefined 上崩溃。
    #[serde(default)]
